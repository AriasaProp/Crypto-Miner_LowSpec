            ...
            synchronized (mService) {
              if (stateMiningUpdate != mService.state)
                statusHandler.sendMessage(
                    statusHandler.obtainMessage(MSG_STATE, stateMiningUpdate = mService.state, 0));
              if (!mService.console.isEmpty()) {
                for (ConsoleItem ci : mService.console) logList.add(0, ci);
                while (logList.size() > MAX_LOG_COUNT) logList.remove(logList.size() - 1);
                mService.console.clear();
                statusHandler.sendMessage(
                    statusHandler.obtainMessage(MSG_UPDATE, MSG_UPDATE_CONSOLE, 0));
              }
              if (mService.minerStatus[STATUS_TYPE_SPEED] != null) {
                statusHandler.sendMessage(
                    statusHandler.obtainMessage(
                        MSG_UPDATE, MSG_UPDATE_SPEED, 0, mService.minerStatus[STATUS_TYPE_SPEED]));
